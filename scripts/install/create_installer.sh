...
# DualVPN Manager Installer Creation Script
# 创建专业安装程序(.exe/.dmg)的脚本

echo "========================================="
echo "  DualVPN Manager Installer Creator     "
echo "========================================="
echo
...
if [ "$OS_TYPE" = "Darwin" ]; then
    ...
    # 创建DMG安装程序
    MACOS_DMG_NAME="dualvpn_manager-macos-$VERSION.dmg"
    echo "创建macOS DMG安装程序: $MACOS_DMG_NAME"
    
    create-dmg \
      --volname "DualVPN Manager" \
      --window-pos 200 120 \
      --window-size 800 400 \
      --icon-size 100 \
    ...
    # 创建安装脚本
    INSTALL_SCRIPT="$OUTPUT_DIR/install-dualvpn-manager.sh"
    cat > "$INSTALL_SCRIPT" << 'EOF'
#!/bin/bash

# DualVPN Manager Linux Installer
# Linux安装脚本

echo "DualVPN Manager Linux 安装程序"
echo "=============================="
echo
...
# 创建安装目录
INSTALL_DIR="/opt/dualvpn_manager"
...
# 创建桌面快捷方式
DESKTOP_FILE="/usr/share/applications/dualvpn_manager.desktop"
echo "创建桌面快捷方式..."
cat > "$DESKTOP_FILE" << 'DESKTOP_EOF'
[Desktop Entry]
Name=DualVPN Manager
Comment=DualVPN Manager - 同时管理OpenVPN和ClashX连接的轻量级工具
Exec=/opt/dualvpn_manager/dualvpn_manager
Icon=/opt/dualvpn_manager/data/flutter_assets/images/icon.png
Terminal=false
Type=Application
...
# 创建启动脚本
LAUNCH_SCRIPT="/usr/local/bin/dualvpn_manager"
echo "创建启动脚本..."
...
chmod +x "$INSTALL_DIR/dualvpn_manager"
chmod -R 755 "$INSTALL_DIR"

echo "安装完成！"
echo "您可以通过以下方式启动应用:"
echo "  1. 在应用菜单中找到DualVPN Manager"
echo "  2. 在终端中运行: dualvpn_manager"
echo "  3. 运行: /opt/dualvpn_manager/dualvpn_manager"
EOF
    ...
    if [[ "$OSTYPE" == "msys" ]] || [[ "$OSTYPE" == "win32" ]] || [[ -n "$WINDIR" ]]; then
        ...
        # 创建Inno Setup脚本
        INNO_SCRIPT="$OUTPUT_DIR/dualvpn_manager_installer.iss"
        # 使用相对于项目根目录的正确路径
...
#define MyAppName "DualVPN Manager"
#define MyAppVersion "$VERSION"
#define MyAppPublisher "V8EN"
#define MyAppURL "https://v8en.com"
#define MyAppExeName "dualvpn_manager.exe"
...
OutputDir=.
OutputBaseFilename=dualvpn_manager-windows-{#MyAppVersion}-setup
...
EOF
        ...
        if [ $? -eq 0 ]; then
            WINDOWS_INSTALLER=$(ls "$OUTPUT_DIR"/dualvpn_manager-windows-*.exe 2>/dev/null | head -n 1)
