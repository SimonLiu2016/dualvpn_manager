...
# DualVPN Manager Release Build Script
# 一键打包Windows和macOS版本

echo "========================================="
echo "  DualVPN Manager Release Build Script  "
echo "========================================="
echo
...
if [ $? -eq 0 ]; then
    ...
    if [ -d "$MACOS_BUILD_DIR" ]; then
        ...
        if [ -n "$MACOS_APP_FILE" ] && [ -d "$MACOS_APP_FILE" ]; then
            ...
            # 先创建ZIP文件作为临时方案
            cd "$MACOS_BUILD_DIR"
            zip -r "$OUTPUT_DIR/dualvpn_manager-macos-$VERSION.zip" ./* >/dev/null 2>&1
            
            if [ $? -eq 0 ]; then
                echo "✓ macOS ZIP包创建成功: $OUTPUT_DIR/dualvpn_manager-macos-$VERSION.zip"
                MACOS_SIZE=$(du -h "$OUTPUT_DIR/dualvpn_manager-macos-$VERSION.zip" | cut -f1)
