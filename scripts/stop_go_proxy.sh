...
# Go代理核心服务停止脚本
# 用于停止正在运行的dualvpn-proxy进程

echo "正在停止Go代理核心服务..."

# 查找并终止dualvpn-proxy进程
PIDS=$(pgrep -f "dualvpn-proxy")
if [ ! -z "$PIDS" ]; then
    echo "找到dualvpn-proxy进程，正在终止..."
    for PID in $PIDS; do
        echo "  终止进程 PID: $PID"
        kill $PID
    done
    ...
    echo "未找到运行中的dualvpn-proxy进程"
fi

# 查找并终止通过go run启动的进程
GO_RUN_PIDS=$(pgrep -f "go-build.*dualvpn")
